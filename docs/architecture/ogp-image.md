...
認証済みユーザーが公開日記ページ (`/d/:id`) を閲覧すると、ヘッダーに「OGP確認」ボタンが表示される。クリックすると `/api/og/:id` を新しいタブで開き、OGP 画像の描画状態を目視確認できる。

このアクセスにより CDN キャッシュも温まるため、X にシェアする前に押しておくことでクローラが確実にキャッシュ済みの PNG を取得できる。
